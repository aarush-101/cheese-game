...
export class PlayerInput {
  constructor(canvas, onLockChange, onError) {
    this.canvas=canvas;this.keys=new Set();this.yaw=0;this.pitch=0;this.sensitivity=1;this.throwQueued=false;this.jumpQueued=false;
    document.addEventListener('pointerlockchange',()=>{
      this.keys.clear();this.throwQueued=false;this.jumpQueued=false;onLockChange(this.locked);
    });
  ...
  reset(player){this.keys.clear();this.throwQueued=false;this.jumpQueued=false;this.yaw=player.yaw;this.pitch=player.pitch;}
  sample(){
    if(!this.locked)return{};
    ...
    const command={
      ...
      yaw:this.yaw,pitch:this.pitch,sprint:this.keys.has('ShiftLeft')||this.keys.has('ShiftRight'),throw:this.throwQueued,jump:this.jumpQueued,climb:forward,
    };
