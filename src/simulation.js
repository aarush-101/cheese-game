...
export function createGameState(seed = 12345, { controllers = ['human', 'bot'], mapId = DEFAULT_MAP_ID } = {}) {
  ...
  const state = {
    ...
    bases: map.bases.map(b => ({ ...b, poisonedUntil: 0, count: 0 })),
    rats: [], pickups: map.pickups.map((p, id) => ({ ...p, id, availableAt: 0 })),
...
function getAttractors(state) {
  return [
    ...state.players.map(p => ({ x: p.x, y: p.y, z: p.z, cheese: p.cheese, key: `player:${p.id}` })),
    ...state.bases.map(b => ({ x: b.x, y: b.y, z: b.z, cheese: b.poisonedUntil > state.time ? 0 : RULES.baseCheese, key: `base:${b.id}` })),
  ];
}
...
function routeDirection(state, body, target, radius, interval = 0.65) {
  ...
  // Route toward the support below airborne targets, not an unreachable point.
  target = { ...target, y: floorHeight(target.x, target.z, (target.y ?? 0) + 0.05, map) };
  if (clearPath(body, target, radius, map)) {
    body.path = [];
  ...
  return d > 0.01 ? { x: (point.x - body.x) / d, z: (point.z - body.z) / d } : { x: 0, z: 0 };
}
function updateRats(state, dt) {
  const attractors = getAttractors(state), map = getMap(state.mapId);
  for (const rat of state.rats) {
    ...
    if (rat.fleeUntil > state.time) {
      ...
      if (home && home.poisonedUntil <= state.time) {
        ...
        target = { y: home.y, x: home.x + Math.sin(angle) * 1.85, z: home.z + Math.cos(angle) * 1.85 };
      ...
      if (!target) {
        if (!rat.wander || state.time >= rat.nextWander || distance(rat, rat.wander) < 0.5) {
          ...
          const x = clamp(rat.x + Math.cos(angle) * length, -map.half + 2, map.half - 2), z = clamp(rat.z + Math.sin(angle) * length, -map.half + 2, map.half - 2);
          const y = floorHeight(x,z,rat.y+0.3,map);
          rat.wander = isFree(x,z,0.3,map,y) ? { x,y,z } : { x:rat.x,y:rat.y,z:rat.z };
...
export function poisonBase(state, baseId) {
  const base = state.bases[baseId], map = getMap(state.mapId);
  ...
  for (const rat of state.rats) if (rat.capturedBy === baseId || distance(rat, base) <= ARENA.baseRadius) {
    ...
    for (let i = 0; i < 8; i++) {
      if (isFree(rat.x + Math.cos(angle) * 8, rat.z + Math.sin(angle) * 8, 0.3, map, rat.y)) break;
...
function updateProjectiles(state, dt) {
  state.projectiles = state.projectiles.filter(p => {
    ...
    for (let i = 0; i < steps; i++) {
      ...
      const hit = projectileHits(p, getMap(state.mapId));
      if (hit) {
...
function nearestRatCluster(state, player) {
  const rats = state.rats.filter(r => r.capturedBy === null && r.fleeUntil <= state.time && r.target !== `player:${player.id}`);
  ...
  for (const rat of rats) {
    ...
    if (score > bestScore) {
      bestScore = score;
      best = { y: rat.y, x: neighbors.reduce((n, r) => n + r.x, 0) / neighbors.length, z: neighbors.reduce((n, r) => n + r.z, 0) / neighbors.length };
    }
  }
  return best;
}
export function botCommand(state, player) {
  ...
  if (state.time >= ai.nextDecision) {
    ...
    if (player.cheese < 30 || (ai.mode === 'REFILL' && player.cheese < 99)) {
      ...
      ai.target = { y: enemy.y, x: enemy.x + vx * 11, z: enemy.z + vz * 11 };
      if (d >= 8 && d <= 12.5 && clearPath(player, enemy, 0.2, getMap(state.mapId))) {
        player.yaw = Math.atan2(-(enemy.x - player.x), -(enemy.z - player.z));
        player.pitch = -0.05 + (random(state) - 0.5) * 0.06;
        throwNow = true;
      } else if (d < 8) ai.target = { y: enemy.y, x: enemy.x + vx * 10, z: enemy.z + vz * 10 };
    } else if (!player.poison && pickup) {
      ai.mode = 'INTERRUPT'; ai.target = { x: pickup.x, y: pickup.y, z: pickup.z };
      ...
      ai.mode = 'INTERRUPT'; ai.target = { x: nextPickup.x, y: nextPickup.y, z: nextPickup.z };
    } else if (ai.mode === 'RETURN' || followers.length >= 2 || (followers.length > 0 && followers.length === state.rats.filter(r=>r.capturedBy===null).length)) {
...
function updateCaptures(state) {
  ...
  for (const rat of state.rats) {
    ...
    const base = secured && secured.poisonedUntil <= state.time ? secured :
      rat.fleeUntil > state.time ? null : state.bases.find(b => b.poisonedUntil <= state.time && distance(rat, b) <= ARENA.baseRadius && Math.abs(rat.y-b.y)<1);
